...
plot.dieharder <- function(x, ...) {
  ...
  local.par <- par(mfrow=c(2,1), mar=c(2,2,3,1), oma=c(0,0,2.5,0))

  vec <- x$data

  hist(vec, probability=TRUE, main="Histogram and Density estimate", xlab="")
  ...
  mtext(text=paste(x$method, "applied to data generated by", x$generator),
